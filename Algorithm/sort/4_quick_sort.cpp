...
void quick_sort(int arr[], int start, int end)
{
	...
	{
            if(arr[i] < arr[pivot]) { // start에서 pivot 보다 큰 값을 찾을 때까지 index 이동
		i++;
		continue;
	    }
	
	    if(arr[j] > arr[pivot]) { // end에서 pivot보다 작은 값을 찾을 때까지 index 이동
		j--;		
		continue;
	    }
			
	    /* start에서 pivot 보다 큰 값을 찾고, end에서 pivot보다 작은 값을 찾았을 때 swap */
	    my_swap(&arr[i], &arr[j]);
            i++;
            j--;
        }
	
	/* index 간 교차가 일어난, 즉, 시작점 인덱스보다 끝점 인덱스가 작은 경우 */
